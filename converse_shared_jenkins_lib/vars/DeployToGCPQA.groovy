def call(String serviceName, String qaClusterNamespace ){
  ansiColor('xterm') {
    unstash 'git-repo'
    sshagent(['adopssh']) {
        withCredentials([[$class: 'FileBinding', credentialsId: 'qasrv12va', variable: 'GOOGLE_APPLICATION_CREDENTIALS']]) {
            sh '''#!/bin/bash -eux     
            ...
            ~/google-cloud-sdk/bin/gcloud container clusters get-credentials ${QA_GKE_CLUSTER_NAME} --region us-east4 --project dol-virtualassistant-qa
            ...
            ./linux-amd64/helm upgrade -f ./jenkins-helm/''' + serviceName + '''/values-qa.yaml ''' + serviceName + '''-qa --install --namespace ''' + qaClusterNamespace + ''' ./jenkins-helm/''' + serviceName + ''' --set image.repository="us.gcr.io/dol-virtualassistant-qa/converse3/$SERVICE_NAME" --set image.tag=$BUILD_NUMBER-$COMMIT_ID --set tracing.enabled=true
            #echo $?
