  ...
  ansiColor('xterm') {
    ...
    sshagent(['adopssh']) {
        withCredentials([[$class: 'FileBinding', credentialsId: 'testsrv12va', variable: 'GOOGLE_APPLICATION_CREDENTIALS']]) {
            ...
            /root/google-cloud-sdk/bin/gcloud container clusters get-credentials ${TEST_EKS_CLUSTER_NAME} --region us-east4 --project acn-newyork-dol-gcp
            ...
            helm upgrade -f ./jenkins-helm/''' + serviceName + '''/values-testque.yaml ''' + serviceName + '''-test --install --namespace ''' + testClusterNamespace + ''' ./jenkins-helm/''' + serviceName + ''' --set image.repository="us.gcr.io/acn-newyork-dol-gcp/converse3/$SERVICE_NAME" --set image.tag=$BUILD_NUMBER-$COMMIT_ID --set tracing.enabled=true
            #echo $?
