def call(String serviceName, String prodClusterNamespace ){
  ansiColor('xterm') {
    unstash 'git-repo'
    sshagent(['adopssh']) {
        withCredentials([[$class: 'FileBinding', credentialsId: 'prodsrv12va', variable: 'GOOGLE_APPLICATION_CREDENTIALS']]) {
            sh '''#!/bin/bash -eux     
            ...
            ~/google-cloud-sdk/bin/gcloud container clusters get-credentials ${PROD_GKE_CLUSTER_NAME} --region us-east4 --project dol-virtualassistant-prod
            ...
            ./linux-amd64/helm upgrade -f ./jenkins-helm/''' + serviceName + '''/values-production.yaml ''' + serviceName + ''' --install --namespace ''' + prodClusterNamespace + ''' ./jenkins-helm/''' + serviceName + ''' --set image.repository="us.gcr.io/dol-virtualassistant-prod/converse3/$SERVICE_NAME" --set image.tag=$BUILD_NUMBER-$COMMIT_ID --set tracing.enabled=true
            #echo $?
