def call(String serviceName, String testClusterNamespace ){
  ansiColor('xterm') {
    unstash 'git-repo'
    sshagent(['adopssh']) {
        withCredentials([[$class: 'FileBinding', credentialsId: 'testsrv12va', variable: 'GOOGLE_APPLICATION_CREDENTIALS']]) {
            sh '''#!/bin/bash -eux     
            ...
            ~/google-cloud-sdk/bin/gcloud container clusters get-credentials ${TEST_EKS_CLUSTER_NAME} --region us-east4 --project acn-newyork-dol-gcp
            ...
            ./linux-amd64/helm upgrade -f ./jenkins-helm/''' + serviceName + '''/values-test.yaml ''' + serviceName + '''-test --install --namespace ''' + testClusterNamespace + ''' ./jenkins-helm/''' + serviceName + ''' --set image.repository="us.gcr.io/acn-newyork-dol-gcp/converse3/$SERVICE_NAME" --set image.tag=$BUILD_NUMBER-$COMMIT_ID --set tracing.enabled=true
            #echo $?
