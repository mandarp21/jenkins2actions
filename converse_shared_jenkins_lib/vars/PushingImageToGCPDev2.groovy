    ...
    ansiColor('xterm') {
        ...
        script {
            COMMIT_ID = readFile('commit.properties').trim()
                ...
                docker.build("us.gcr.io/dol-virtualassistant-dev/converse3/${serviceName}", "--no-cache .")
                docker.image("us.gcr.io/dol-virtualassistant-dev/converse3/${serviceName}").push("${BUILD_NUMBER}-$COMMIT_ID")
            }
        }
    }
}
